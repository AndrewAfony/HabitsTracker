...
class NewViewModelTest {
    ...
    fun save_first_card() {
        ...
        assertEquals(NewUiState.Replace(position = 0, card = Card.ZeroDays("days without smoking")), communication.list[2])
    ...
    fun test_edit_zero_days_card_and_cancel() {
        val communication = FakeCommunication()
        val interactor = FakeInteractor(listOf(Card.ZeroDays(text = "days without smoking"), Card.Add))
        val viewModel = NewViewModel(communication, interactor)
        viewModel.init(isFirstRun = true)

        assertEquals(NewUiState.Add(Card.ZeroDays(text = "days without smoking"), Card.Add), communication.list[0])
        assertEquals(1, communication.list.size)

        viewModel.editZeroDaysCard(position = 0)
        assertEquals(NewUiState.Replace(position = 0, card = Card.ZeroDays("days without smoking")), 1)
